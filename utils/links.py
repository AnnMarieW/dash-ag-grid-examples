

# External links and images
forum = "https://community.plotly.com/"
ag_grid_docs = "https://www.ag-grid.com/react-data-grid/"
ag_grid_docs_img = "https://github.com/AnnMarieW/HelloDash/assets/72614349/d6b2276f-f3f9-4813-8a34-f96353c43687"
dash_ag_grid_github = "https://github.com/plotly/dash-ag-grid"
dag_docs = "https://dash.plotly.com/dash-ag-grid"
dag_docs_img = "https://github.com/AnnMarieW/HelloDash/assets/72614349/978dfcdc-7510-42ac-a562-0b8e85132a0e"

get_started_V31_img = "https://github.com/AnnMarieW/dash-ag-grid-examples/assets/72614349/21c45aff-9a9b-4cf2-9e27-8e598efd4074"
get_started_easy_img = "https://github.com/AnnMarieW/dash-ag-grid-examples/assets/72614349/4f358545-ff20-4f68-9009-0675508c249f"

under_construction_img = "https://github.com/AnnMarieW/dash-ag-grid-examples/assets/72614349/0aaaf2dc-51aa-4ca5-bda8-f7e2e3a53b87"
examples_img = "https://github.com/AnnMarieW/dash-ag-grid-examples/assets/72614349/9d46bfd7-0fe1-4a55-a63b-e935c7ca8d5e"

theme_switch_img = "https://github.com/AnnMarieW/dash-ag-grid-examples/assets/72614349/6aaa5b0c-2200-4594-95af-f6bffe849235"



# Page links
get_started_V31 = "/get-started-v31"
get_started_easy = "/get-started-easy"
theme_switch = "/theme-switch"
components_dbc= "/components-dbc"
components_dcc= "/components-dcc"
components_dmc= "/components-dmc"
components_html= "/components-html"
import_export="/import-export"
styling="/styling"






# Notes:
"""
Format for internal links in dcc.Markdown:
     <dccLink href="/" children="Home" />

-----------------
Starting with Dash AG Grid 31.0 the Dash version numbers will match the version number of the AG Grid component it wraps. This will make it easier to know which version of the AG Grid documentation to use if you need more information.

To see a previous version of the AG Grid docs, use a link like:
https://www.ag-grid.com/archive/<version number>

For example:
https://www.ag-grid.com/archive/29.3.5

----------------
"""


